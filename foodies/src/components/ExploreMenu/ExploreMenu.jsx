...
const ExploreMenu = ({category, setCategory}) => {
  ...
  return (
    <div className="explore-menu position-relative">
      ...
      <p>Explore Curated lists of dishes from top cataegories</p>
          ...
          categories.map((item, index) => {
             return(
              <div key={index} className="text-center explore-menu-list-item">
                <img src={item.icon} alt="" className={item.category === category ? 'rounded-circle active' : 'rounded-circle'} onClick={() => setCategory(prev => prev=== item.category ? 'All' : item.category)} height={110} width={110} />
